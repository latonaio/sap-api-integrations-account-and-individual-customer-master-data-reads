...
import (
	"encoding/json"
	"sap-api-integrations-account-and-individual-customer-master-data-reads/SAP_API_Caller/responses"
...
func ConvertToCorporateAccountCollection(raw []byte, l *logger.Logger) ([]CorporateAccountCollection, error) {
	pm := &responses.CorporateAccountCollection{}

	err := json.Unmarshal(raw, pm)
	if err != nil {
		return nil, xerrors.Errorf("cannot convert to CorporateAccountCollection. unmarshal error: %w", err)
	}
	if len(pm.D.Results) == 0 {
		return nil, xerrors.New("Result data is not exist")
	}
	...
	corporateAccountCollection := make([]CorporateAccountCollection, 0, 10)
	for i := 0; i < 10 && i < len(pm.D.Results); i++ {
		data := pm.D.Results[i]
		corporateAccountCollection = append(corporateAccountCollection, CorporateAccountCollection{
			ObjectID:                          data.ObjectID,
			AccountID:                         data.AccountID,
			UUID:                              data.UUID,
			ExternalID:                        data.ExternalID,
	...
	return corporateAccountCollection, nil
}
...
func ConvertToIndividualCustomerAddress(raw []byte, l *logger.Logger) ([]IndividualCustomerAddress, error) {
	pm := &responses.IndividualCustomerAddress{}
